...
void renderKernel(const int NI,
  ...
  /* for Q5, we need to split the processes among the (size) of ranks
     so the outer loop now iterates through the height by just one rank
     that goes from the pixel that the end of rank covers to the pixel 
     that the beginning of the rank covers; i.e. in reverse step so the image
     (upside down through the lens) can be reverted in the end
   */
  int rank, size;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  
  int end_J = (size-rank-1)*NJ/size;
  int start_J = (size - rank)*NJ/size;
  ...
  printf("looping over all pixels complete.\n");
  MPI_Barrier(MPI_COMM_WORLD); // Blocks until all processes in the comminicator reach this routine
}
